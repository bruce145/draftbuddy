 ...
 */
public class NHLStatAPILiveFeedParser {

    private static String dbConnectionString = "";

    private static Connection conn;

    public NHLStatAPILiveFeedParser(String connection){
...
//---------------------------GRAB JSON FILES--------------------------------------//
    ...
    private void gameGrabber(String gameID){
        ...
        saveStringToFile(jsonString, gameID);


    }

    private void saveStringToFile(String s, String fname){
        String saveDirectory = "src/liveGame/2017-18/";

        try {
            BufferedWriter out = new BufferedWriter(new FileWriter(saveDirectory + fname + ".json"));
            out.write(s);
            out.close();
...
//----------------------------UPDATE EXISTING RECORDS--------------------------//
    ...
    private void bulkUpdate(int newGameID){

        int lastGame = getLastDownloadedGameNo();

            for(int i = 1; i <= 6; i++) {
                operationSelect(i, 17, lastGame + 1, newGameID);
            }
    }
 ...
 //---------------------------Helper Functions-----------------------------------//
    ...
    private int findLatestGameNo() {
        int g = getLastDownloadedGameNo();
        String jsonString = null;
        JsonFactory jFactory = new JsonFactory();
        JsonParser jParse;
        JsonNode rootNode;
        try {
            for(int i = g; i < 1230; i++) {
...
//-------------------------DB and user Interface--------------------------------//
    ...
    private void operationSelect(int selection, int season, int startGame, int endGame){
        ...
        switch (selection) {
            ...
            case 1:
                ...
                for(int i = lastGame + 1; i <= endGame; i++) {
                    String format = String.format("%04d", i);
    ...
    private void autoUpdate(){
        ...
        int game = findLatestGameNo();
        String response = (game == getLastDownloadedGameNo()) ? "All games up to date" : "Latest available game is " + game;
    ...
    public static void main(String args[]){
        NHLStatAPILiveFeedParser n = new NHLStatAPILiveFeedParser(grabConnectionString());
        n.autoUpdate();
        //System.out.println(grabConnectionString());
    }
}
